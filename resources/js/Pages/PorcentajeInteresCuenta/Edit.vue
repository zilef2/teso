...
const props = defineProps({
    show: Boolean,
    title: String,
    PorcentajeInteresCuentaa: Object,
...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.PorcentajeInteresCuentaa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.PorcentajeInteresCuentaa[names['order']]
        });

        // form.codigo = props.PorcentajeInteresCuentaa?.codigo
    }
})

const update = () => {
    form.put(route('PorcentajeInteresCuenta.update', props.PorcentajeInteresCuentaa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.PorcentajeInteresCuentaa.PorcentajeInteresCuenta_id]"></v-select>
