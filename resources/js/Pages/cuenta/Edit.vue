...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.cuentaa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.cuentaa[names['order']]
        });

        // form.codigo = props.cuentaa?.codigo
    }
})

const update = () => {
    form.put(route('cuenta.update', props.cuentaa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.cuentaa.cuenta_id]"></v-select>
