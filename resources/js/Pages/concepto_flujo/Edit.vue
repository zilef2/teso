...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.concepto_flujoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.concepto_flujoa[names['order']]
        });

        // form.codigo = props.concepto_flujoa?.codigo
    }
})

const update = () => {
    form.put(route('concepto_flujo.update', props.concepto_flujoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.concepto_flujoa.concepto_flujo_id]"></v-select>
