...
import Create from '@/Pages/concepto_flujo/Create.vue';
import Edit from '@/Pages/concepto_flujo/Edit.vue';
import Delete from '@/Pages/concepto_flujo/Delete.vue';
...
watch(() => _.cloneDeep(data.params), debounce(() => {
    ...
    router.get(route("concepto_flujo.index"), params, {
        replace: true,
        preserveState: true,
        preserveScroll: true,
    })
...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.fromController?.data.forEach((concepto_flujo) => {
            data.selectedId.push(concepto_flujo.id)
        })
    }
}
const select = () => {
        ...
        <div class="space-y-4">
            ...
            <div class="px-4 sm:px-0">
                <div class="rounded-lg overflow-hidden w-fit">
                    ...
                    <Edit v-if="can(['update concepto_flujo'])" :titulos="titulos"
                        :numberPermissions="props.numberPermissions" :show="data.editOpen" @close="data.editOpen = false"
                        :concepto_flujoa="data.concepto_flujoo" :title="props.title" :losSelect=props.losSelect />

                    <Delete v-if="can(['delete concepto_flujo'])" :numberPermissions="props.numberPermissions"
                        :show="data.deleteOpen" @close="data.deleteOpen = false" :concepto_flujoa="data.concepto_flujoo"
                        :title="props.title" />
                </div>
            </div>
                        ...
                        <tbody>
                            <tr v-for="(claseFromController, indexu) in props.fromController.data" :key="indexu"
                                ...
                                <td v-if="numberPermissions > 1" class="whitespace-nowrap py-4 w-12 px-2 sm:py-3">
                                    <div class="flex justify-center items-center">
                                        <div class="rounded-md overflow-hidden">
                                            <InfoButton v-show="can(['update concepto_flujo'])" type="button"
                                                @click="(data.editOpen = true), (data.concepto_flujoo = claseFromController)"
                                                class="px-2 py-1.5 rounded-none" v-tooltip="lang().tooltip.edit">
                                                <PencilIcon class="w-4 h-4" />
                                            </InfoButton>
                                            <DangerButton v-show="can(['delete concepto_flujo'])" type="button"
                                                @click="(data.deleteOpen = true), (data.concepto_flujoo = claseFromController)"
                                ...
                                <td v-for="titulo in titulos" class="whitespace-nowrap py-4 px-2 sm:py-3">
                                    ...
                                    <span v-if="titulo['type'] === 'foreign'"> {{ claseconcepto_flujoa[titulo['order']][titulo['nameid']] }} </span>
                                </td>
