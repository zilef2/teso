...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.Seguimientoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.Seguimientoa[names['order']]
        });

        // form.codigo = props.Seguimientoa?.codigo
    }
})

const update = () => {
    form.put(route('Seguimiento.update', props.Seguimientoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.Seguimientoa.Seguimiento_id]"></v-select>
