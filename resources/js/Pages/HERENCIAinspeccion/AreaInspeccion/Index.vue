...
import Create from '@/Pages/AreaInspeccion/Create.vue';
import Edit from '@/Pages/AreaInspeccion/Edit.vue';
import Delete from '@/Pages/AreaInspeccion/Delete.vue';
...
watch(() => _.cloneDeep(data.params), debounce(() => {
    ...
    router.get(route("AreaInspeccion.index"), params, {
        replace: true,
        preserveState: true,
        preserveScroll: true,
    })
...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.AreaInspeccions?.data.forEach((AreaInspeccion) => {
            data.selectedId.push(AreaInspeccion.id)
        })
    }
}
const select = () => {
        ...
        <div class="space-y-4">
            ...
            <div class="px-4 sm:px-0">
                <div class="rounded-lg overflow-hidden w-fit">
                    ...
                    <Create v-if="can(['create area'])" :numberPermissions="props.numberPermissions"
...
<!--                    v-if="can(['update area'])"-->
                    <Edit :titulos="titulos"
                        ...
                        :AreaInspecciona="data.AreaInspecciono" :title="props.title" :losSelect=props.losSelect />
                    <Delete v-if="can(['delete area'])" :numberPermissions="props.numberPermissions"
                        :show="data.deleteOpen" @close="data.deleteOpen = false" :AreaInspecciona="data.AreaInspecciono"
                        :title="props.title" />
                </div>
            </div>
                ...
                <div class="flex justify-between p-2">
                    <div class="flex space-x-2">
                        ...
                        <!-- <DangerButton @click="data.deleteBulkOpen = true"
                            v-show="data.selectedId.length != 0 && can(['delete AreaInspeccion'])" class="px-3 py-1.5"
...
<!--                                <th v-if="numberPermissions > 1" class="px-2 py-4">Accion</th>-->
                                ...
                                <th v-for="titulo in titulos" class="px-6 py-4 cursor-pointer"
                                ...
                                <td v-if="numberPermissions > 1" class="whitespace-nowrap py-4 w-12 px-2 sm:py-3">
                                    <div class="flex justify-center items-center">
                                        <div class="rounded-md overflow-hidden">
                                            <InfoButton v-show="can(['update area'])" type="button"
                                                @click="(data.editOpen = true), (data.AreaInspecciono = claseFromController)"
                                                class="px-2 py-1.5 rounded-none" v-tooltip="lang().tooltip.edit">
                                                <PencilIcon class="w-4 h-4" />
                                            </InfoButton>
                                            <DangerButton v-show="can(['delete area'])" type="button"
                                                @click="(data.deleteOpen = true), (data.AreaInspecciono = claseFromController)"
...
<!--                                <td class="whitespace-nowrap py-4 px-2 sm:py-3 text-center">{{ ++indexu }}</td>-->
                                <td v-for="titulo in titulos" class="whitespace-nowrap py-4 px-6 sm:py-3">
                                    ...
                                    <span v-if="titulo['type'] === 'foreign'"> {{ claseAreaInspecciona[titulo['order']][titulo['nameid']] }} </span>
                                </td>
