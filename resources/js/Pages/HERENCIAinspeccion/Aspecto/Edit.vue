...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.Aspectoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.Aspectoa[names['order']]
        });

        // form.codigo = props.Aspectoa?.codigo
    }
})

const update = () => {
    form.put(route('Aspecto.update', props.Aspectoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.Aspectoa.Aspecto_id]"></v-select>
