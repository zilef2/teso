...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.inspecciona[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.inspecciona[names['order']]
        });

        // form.codigo = props.inspecciona?.codigo
    }
})

const update = () => {
    form.post(route('inspeccion.update', props.inspecciona?.id), {
    // form.put(route('inspeccion.update', props.inspecciona?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.inspecciona.inspeccion_id]"></v-select>
