...
import InstantVideo from "@/Pages/registrosmedio/InstantVideo.vue";
...
onMounted(() => {
    if (props.numberPermissions > 900) {
        ...
        // form.nombre = 'nombre de prueba registrosmedio ' + (valueRAn);
        // form.hora_inicial = '0'+valueRAn+':00'//temp

        let hoy = TransformTdate(new Date(), true)
        form.fecha_realizacion = hoy
        form.userRecibir = 'ejemplo Recibe: ' + hoy
        form.userEntregar = 'ejemplo entregar'
        form.adjetivo = 'ejemplo adjetivo'
        form.placa = 'p40279'
        form.categoria = 'ejemplo categoria'
    }
});
...
const create = () => { //this is the most recent version of the function
    ...
    form.post(route('registrosmedio.store', [props.idregistrosmedio, form.contador]), {
...
const photoCapturedHandler = (photoData) => {
    form.registrofotografico = photoData;
};
                ...
                <section class="relative flex h-32 items-end bg-gray-900 lg:col-span-3 lg:h-full xl:col-span-3">
                    ...
                    <div class="hidden lg:relative lg:block lg:p-12">
                        ...
                        <h2 class="mt-6 text-2xl font-bold text-white sm:text-3xl md:text-4xl">
                            Registro fotografico para medios audiovisuales
                        </h2>
                    ...
                    <div class="grow">
                        <main class="flex-grow items-center justify-center px-0 py-8 lg:col-span-9 lg:py-4 xl:col-span-9">
                            <div class="w-full lg:min-w-xl">
                                ...
                                <form @submit.prevent="create" class="mt-8 grid grid-cols-2 xl:grid-cols-3 gap-6">
                                    ...
                                    <div class="">
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Responsable de recibir la
                                            inspección</label>
                                        <TextInput id="userRecibir" type="text" class="mt-1 block w-full" v-model="form.userRecibir" required
                                                   :error="form.errors.userRecibir"/>
                                    </div>
                                    <div class="">
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Responsable de recibir la
                                            inspección</label>
                                        <TextInput id="userEntregar" type="text" class="mt-1 block w-full" v-model="form.userEntregar" required
                                                   :error="form.errors.userEntregar"/>
                                    </div>
                                    <div class="">
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Responsable de recibir la
                                            inspección</label>
                                        <TextInput id="adjetivo" type="text" class="mt-1 block w-full" v-model="form.adjetivo" required
                                                   :error="form.errors.adjetivo"/>
                                    </div>
                                    <div class="">
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Responsable de recibir la
                                            inspección</label>
                                    ...
                                    <div class="col-span-full xl:col-span-3 sm:items-center sm:gap-4 mt-2 mb-16 xl:mb-6">
