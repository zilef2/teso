...
import ShowAspectos from "@/Pages/registrosmedio/ShowAspectos.vue";
import SeccionTitular from "@/Pages/registrosmedio/SeccionTitular.vue";
import SeccionTitularGrande from "@/Pages/registrosmedio/SeccionTitularGrande.vue";
...
const props = defineProps({
    ...
    losSelect: Object,
    registrosmedios: Object,
    responsable: Array,
    arearegistrosmedio: Object,
    aspectos: Object,
    //only in edit
})
...
onMounted(() => {
    ...
    form.fecha_realizacion = TransformTdate(props.registrosmedios.fecha_realizacion, true)
...
const update = () => {
    ...
    form.put(route('registrosmedio.update', props.registrosmedios.id), {
        // preserveScroll: true,
        onSuccess: () => {
            // emit("close")
            form.reset()
                                    ...
                                    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                                        <div class="xl:col-span-3 w-full">
                                            <label for="FirstName" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Área de
                                                inspección</label>
                                            <div id="arearegistrosmedio" class="mt-1 ">
                                                <vSelect multiple v-model="form.arearegistrosmedio"
                                                         :options="props.losSelect['areas']"
                                                >
                                                </vSelect>
                                                <!--                                                hola : {{props.losSelect['areas']}}-->

                                                <InputError class="mt-2" :message="form.errors.arearegistrosmedio"/>
                                            </div>
                                        </div>
                                        <!--                                        qwe: {{ form.arearegistrosmedio }}-->
