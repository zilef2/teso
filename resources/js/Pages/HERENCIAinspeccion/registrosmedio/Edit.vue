...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.registrosmedioa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.registrosmedioa[names['order']]
        });

        // form.codigo = props.registrosmedioa?.codigo
    }
})

const update = () => {
    form.post(route('registrosmedio.update', props.registrosmedioa?.id), {
    // form.put(route('registrosmedio.update', props.registrosmedioa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.registrosmedioa.registrosmedio_id]"></v-select>
