...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.bancoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.bancoa[names['order']]
        });

        // form.codigo = props.bancoa?.codigo
    }
})

const update = () => {
    form.put(route('banco.update', props.bancoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.bancoa.banco_id]"></v-select>
