...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.Comprobantea[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.Comprobantea[names['order']]
        });

        // form.codigo = props.Comprobantea?.codigo
    }
})

const update = () => {
    form.put(route('Comprobante.update', props.Comprobantea?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.Comprobantea.Comprobante_id]"></v-select>
