...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.sinafectaciona[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.sinafectaciona[names['order']]
        });

        // form.codigo = props.sinafectaciona?.codigo
    }
})

const update = () => {
    form.put(route('sinafectacion.update', props.sinafectaciona?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.sinafectaciona.sinafectacion_id]"></v-select>
