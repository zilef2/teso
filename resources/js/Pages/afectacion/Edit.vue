...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.afectaciona[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.afectaciona[names['order']]
        });

        // form.codigo = props.afectaciona?.codigo
    }
})

const update = () => {
    form.put(route('afectacion.update', props.afectaciona?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.afectaciona.afectacion_id]"></v-select>
