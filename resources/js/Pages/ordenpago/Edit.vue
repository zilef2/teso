...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.ordenpagoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.ordenpagoa[names['order']]
        });

        // form.codigo = props.ordenpagoa?.codigo
    }
})

const update = () => {
    form.put(route('ordenpago.update', props.ordenpagoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.ordenpagoa.ordenpago_id]"></v-select>
