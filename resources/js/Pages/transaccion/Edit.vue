...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.transacciona[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.transacciona[names['order']]
        });

        // form.codigo = props.transacciona?.codigo
    }
})

const update = () => {
    form.put(route('transaccion.update', props.transacciona?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.transacciona.transaccion_id]"></v-select>
