...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.cesinafectaciona[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.cesinafectaciona[names['order']]
        });

        // form.codigo = props.cesinafectaciona?.codigo
    }
})

const update = () => {
    form.put(route('cesinafectacion.update', props.cesinafectaciona?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.cesinafectaciona.cesinafectacion_id]"></v-select>
