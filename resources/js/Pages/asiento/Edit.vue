...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.asientoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.asientoa[names['order']]
        });

        // form.codigo = props.asientoa?.codigo
    }
})

const update = () => {
    form.put(route('asiento.update', props.asientoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.asientoa.asiento_id]"></v-select>
