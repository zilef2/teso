...
import Create from '@/Pages/ceconafectacion/Create.vue';
import Edit from '@/Pages/ceconafectacion/Edit.vue';
import Delete from '@/Pages/ceconafectacion/Delete.vue';
...
watch(() => _.cloneDeep(data.params), debounce(() => {
    ...
    router.get(route("ceconafectacion.index"), params, {
        replace: true,
        preserveState: true,
        preserveScroll: true,
    })
...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.fromController?.data.forEach((ceconafectacion) => {
            data.selectedId.push(ceconafectacion.id)
        })
    }
}
const select = () => {
            ...
            <div class="px-4 sm:px-0">
                <div class="rounded-lg overflow-hidden w-fit">
                    <PrimaryButton class="rounded-none" @click="data.createOpen = true"
                        v-if="can(['create ceconafectacion'])">
                        {{ lang().button.new }}
                    </PrimaryButton>

                    <Create v-if="can(['create ceconafectacion'])" :numberPermissions="props.numberPermissions"
                        :titulos="titulos" :show="data.createOpen" @close="data.createOpen = false" :title="props.title"
                        :losSelect=props.losSelect />

                    <Edit v-if="can(['update ceconafectacion'])" :titulos="titulos"
                        :numberPermissions="props.numberPermissions" :show="data.editOpen" @close="data.editOpen = false"
                        :ceconafectaciona="data.ceconafectaciono" :title="props.title" :losSelect=props.losSelect />

                    <Delete v-if="can(['delete ceconafectacion'])" :numberPermissions="props.numberPermissions"
                        :show="data.deleteOpen" @close="data.deleteOpen = false" :ceconafectaciona="data.ceconafectaciono"
                        :title="props.title" />
                </div>
            </div>
                ...
                <div class="flex justify-between p-2">
                    <div class="flex space-x-2">
                        ...
                        <!-- <DangerButton @click="data.deleteBulkOpen = true"
                            v-show="data.selectedId.length != 0 && can(['delete ceconafectacion'])" class="px-3 py-1.5"
                        ...
                        <tbody>
                            <tr v-for="(claseFromController, indexu) in props.fromController.data" :key="indexu"
                                ...
                                <td v-if="numberPermissions > 1" class="whitespace-nowrap py-4 w-12 px-2 sm:py-3">
                                    <div class="flex justify-center items-center">
                                        <div class="rounded-md overflow-hidden">
                                            <InfoButton v-show="can(['update ceconafectacion'])" type="button"
                                                @click="(data.editOpen = true), (data.ceconafectaciono = claseFromController)"
                                                class="px-2 py-1.5 rounded-none" v-tooltip="lang().tooltip.edit">
                                                <PencilIcon class="w-4 h-4" />
                                            </InfoButton>
                                            <DangerButton v-show="can(['delete ceconafectacion'])" type="button"
                                                @click="(data.deleteOpen = true), (data.ceconafectaciono = claseFromController)"
                                ...
                                <td v-for="titulo in titulos" class="whitespace-nowrap py-4 px-2 sm:py-3">
                                    ...
                                    <span v-if="titulo['type'] === 'foreign'"> {{ clasececonafectaciona[titulo['order']][titulo['nameid']] }} </span>
                                </td>
