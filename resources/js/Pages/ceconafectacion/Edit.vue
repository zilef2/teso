...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.ceconafectaciona[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.ceconafectaciona[names['order']]
        });

        // form.codigo = props.ceconafectaciona?.codigo
    }
})

const update = () => {
    form.put(route('ceconafectacion.update', props.ceconafectaciona?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.ceconafectaciona.ceconafectacion_id]"></v-select>
