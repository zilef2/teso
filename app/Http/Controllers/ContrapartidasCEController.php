    ...
    public function Buscar_CP_CE(Request $request): RedirectResponse
    {
        try {
            ...
            if (!CPhelp::Val_Exista_CE_auxiliar($codigo)) {
                return back()->with('error', 'Faltan archivos. Auxiliar, CE con y sin afectacion');
            }

            DB::beginTransaction();

            $MesTransaccional = Parametro::Where("nombre", "Mes transaccional")->first();
            $Aux_CE = transaccion::Where('codigo', $codigo)
                ->WhereMonth('fecha_elaboracion', $MesTransaccional)
                ->get();


            $this->UPDATEAux_En_SinAfectacion($Aux_CE);
...
//            ->WhereYear('fecha_elaboracion', $anio)
    ...
    public static function AfectacionesCE($codigo, $codigoCuentaBuscada): Collection //toerase
    {
        $valor_debito_credito = (strcmp($codigo, "CI") === 0) ? "valor_debito" : "valor_credito";
        $opuesto_debito_credito = (strcmp($valor_debito_credito, "valor_credito") === 0) ? "valor_debito" : "valor_credito";
        $paraMes = Parametro::Where("nombre", "Mes transaccional")->first();
    ...
    private function UPDATEAux_En_SinAfectacion($Auxiliares)
    {
        foreach ($Auxiliares as $auxCE) {
            $ceSinaf = cesinafectacion::Where('codigo_cuenta', $auxCE->documento)->first();
            if ($ceSinaf) {
                $auxCE->update(['sin_afectacion' => 1]);
            }else{
                $auxCE->update(['sin_afectacion' => 0]);
            }
        }
    }
